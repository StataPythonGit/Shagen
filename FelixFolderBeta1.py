...
def write_master(name=False, temp_dir=False, git_dir=False, analysis=False, file=False):
    f=open("%s.do" %file, "w+")
    if git_dir==False:
        git_dir=""
    if analysis=="analysis":
        basics="""/*************************************************/ \n/*************************************************/ \n/****************  {0}  ***************/ \n/*************************************************/ \n/*************************************************/ \n\nclear \ngraph set print logo off \n \ngraph set print tmargin 1 \ngraph set print lmargin 1\nset more off, perm\nset emptycells drop\n\nclear\nclear matrix\nset matsize 800\n\nset memory 4g\nset varabbrev on\n\n/*********************************************************\n*************         Master File     ********************\n*********************************************************\n\n**********************************\n**Change paths********************\n**********************************/\n
            ...
            \n  global root "{1}"
            \n  }}
            \nelse if regexm(c(os),"Windows") == 1 global root  "%s"
            \n
            \n
            \nglobal do_path "$root/analysis/code"
            \nglobal input_path "$root/analysis/input"
            \nglobal temp_path "$root/analysis/temp"
            \nglobal output_path "$root/analysis/output"
            \nglobal log_path "$root/analysis/log"
            ...
            \n}}\n\n**********************************\n**Run Do-Files********************\n**********************************\n\n\ncd "$log_path"\ncap log close\nlog using preparation${{datum}}, replace\n\ncd "$do_path"\ndo 1-preparation\ncap log close\n\ncd "$log_path"\ncap log close\nlog using regressions${{datum}}, replace\n\ncd "$do_path"\ndo 2-regressions\ncap log close \n\n/**************** Version Controll *****************************/ \n\n shell \"{2}\" --git-dir "$do_path/.git" --work-tree "$do_path/." commit -a -m "version $datum" \n """.format(name, temp_dir ,git_dir)
    if analysis!="analysis":
...
def write_prep(name=False, file=False):
    f=open("%s.do" %file, "w+")
    basics="""/*************************************************/ \n/*************************************************/ \n/****************  %s  ***************/ \n/*************************************************/ \n/*************************************************/ \n\nclear \ngraph set print logo off \n \ngraph set print tmargin 1 \ngraph set print lmargin 1\nset more off, perm\nset emptycells drop\n\nclear\nclear matrix\nset matsize 800\n\nset memory 4g\nset varabbrev on\n\n/*********************************************************\n*************         Preparation File     ********************\n*********************************************************/\n\n\nglobal datum = subinstr(c(current_date)," ","",.)\n\n""" %( name)
    f.write(basics)
    f.close()
...
def write_reg(name=False, file=False):
    f=open("%s.do" %file, "w+")
    basics="""/*************************************************/ \n/*************************************************/ \n/****************  %s  ***************/ \n/*************************************************/ \n/*************************************************/ \n \n \n \n \n \n\nclear \ngraph set print logo off \n \ngraph set print tmargin 1 \ngraph set print lmargin 1\nset more off, perm\nset emptycells drop\n\nclear\nclear matrix\nset matsize 800\n\nset memory 4g\nset varabbrev on\n\n/*********************************************************\n*************         Regressions File     ********************\n*********************************************************/\n\n\nglobal datum = subinstr(c(current_date)," ","",.)\n\n""" %(name)
    f.write(basics)
    f.close()
...
def Gitcommit(git_directory=None, repository=None, files=None, name=None, analysis=False):
    if os.path.isfile(git_directory):
        ...
        for m in files:
            if m=="0-master":
                code=write_master(name, repository, git_directory, analysis, m)
            if m=="1-preparation":
                code=write_prep(name, m)
            if m=="2-regressions":
                code=write_reg(name, m)
    ...
    for j in mains:
        ...
        if os.path.exists(repository):
            ...
            if GitFound!=False:
                ...
                GitFound=Gitcommit(git_location, repository, files, name, j)
                continue
                    ...
                    for name in files:
                        if name == "git.exe" and GitFound==False:
                            ...
                            if place.endswith("bin\git.exe"):
                                ...
                                print place
                                try:
                                    GitFound=Gitcommit(place, repository, files, name, j)
                                    if GitFound!=False:
                                        continue
                                except:
                                    print name
...
#                    print git_location
            ...
            if GitFound==False and sys.platform =='darwin':
                ...
                GitFound=Gitcommit("/usr/local/git/bin/git", repository, files, name, j)
                ...
                while os.path.isfile(git_location)==False:
                    ...
                    if sys.platform =='darwin':
                        ...
                        if  git_location=='q':
                            ...
                            for m in files:
                                if m=="0-master":
                                        code=write_master(name, repository, git_location, j, m)
                                if m=="1-preparation":
                                    code=write_prep(name, m)
                                if m=="2-regressions":
                                        code=write_reg(name, m)
